...
struct AppReducer : RxReducerType {
	func handle(_ action: RxActionType, actionResult: RxActionResultType, currentState: RxStateType) -> Observable<RxStateType> {
		...
		switch action {
		case .loadToDoEntries: return Observable.just(AppState(toDoEntries: (actionResult as! RxDefaultActionResult).value))
		case .addToDoEntry:
		...
		case .deleteToDoEntry:
			...
			return Observable.just(AppState(toDoEntries: currentEntries))
		case .reloadToDoEntries: return Observable.just(AppState(toDoEntries: (actionResult as! RxDefaultActionResult).value))
		}
	}
}
